			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					...
					if($stockStatement->rowCount() > 0){
						...
						$updateSaleDetailsSql = 'UPDATE sale SET itemNumber = :itemNumber, saleDate = :saleDate, unitPrice = :unitPrice, discount = :discount, quantity = :quantity, customerID = :customerID WHERE saleID = :saleID';
						$updateSaleDetailsStatement = $conn->prepare($updateSaleDetailsSql);
						$updateSaleDetailsStatement->execute(['itemNumber' => $saleDetailsItemNumber, 'saleDate' => $saleDetailsSaleDate, 'unitPrice' => $saleDetailsUnitPrice, 'discount' => $saleDetailsDiscount, 'quantity' => $saleDetailsQuantity, 'customerID' => $customerID, 'saleID' => $saleDetailsSaleID]);
