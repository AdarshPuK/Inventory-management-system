			...
			if($stockStatement->rowCount() > 0){
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					if($customerStatement->rowCount() > 0){
						...
						$insertSaleSql = 'INSERT INTO sale(itemNumber, discount, quantity, unitPrice, customerID, saleDate) VALUES(:itemNumber, :discount, :quantity, :unitPrice, :customerID, :saleDate)';
						$insertSaleStatement = $conn->prepare($insertSaleSql);
						$insertSaleStatement->execute(['itemNumber' => $itemNumber, 'discount' => $discount, 'quantity' => $quantity, 'unitPrice' => $unitPrice, 'customerID' => $customerID,  'saleDate' => $saleDate]);
