<?php
	...
	if(isset($_POST['customerDetailsCustomerFullName'])){
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			$sql = 'INSERT INTO customer(fullName, email, mobile, address, city, district, status) VALUES(:fullName, :email, :mobile, :address, :city, :district, :status)';
			$stmt = $conn->prepare($sql);
			$stmt->execute(['fullName' => $fullName, 'email' => $email, 'mobile' => $mobile, 'address' => $address, 'city' => $city, 'district' => $district, 'status' => $status]);
